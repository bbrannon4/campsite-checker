...
## Web app (`docs/`) — GitHub Pages

A static, no-backend companion to the CLI. Enter a ZIP code and a distance, and
it lists recreation.gov campgrounds nearby. Served straight from GitHub Pages
(`docs/` on `main`); everything runs in the browser:

- **`docs/data/campgrounds.json`** — a prebuilt index of ~4,800 US campgrounds
  (name, id, coordinates, site count) generated by `scripts/build_campgrounds.py`.
- **ZIP → coordinates** happens at runtime via the keyless, CORS-open
  [zippopotam.us](https://api.zippopotam.us) service; nearest campgrounds are
  found client-side with a haversine distance filter. No API keys, no server.
...
### Keeping the campground index fresh

Campgrounds open and close, so `.github/workflows/refresh-campgrounds.yml`
rebuilds `campgrounds.json` **quarterly** (Jan/Apr/Jul/Oct 1) and on demand via
the Actions tab's "Run workflow" button. It re-runs the generator and commits
the file only if it changed. The generator uses the same keyless public search
API as the CLI, so the workflow needs no secrets.

To rebuild by hand:

```bash
python scripts/build_campgrounds.py
```

### Not built yet (web app)

Live availability isn't wired into the web app — each result links to its
recreation.gov page for now. Because that API is also CORS-open, a later phase
can fetch availability directly in the browser and reuse the CLI's
weekend/stay-length/reservable logic (ported to JS).
